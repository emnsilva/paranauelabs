...
provider "google" {
  ...
  region      = locals.region_mapping[var.CLOUD_PRIMARY_REGION].gcp
}

# Bucket no Brasil
resource "google_storage_bucket" "brasil" {
  name          = "${var.GCP_PROJECT_ID}-bucket-br"
  location      = locals.region_mapping[var.CLOUD_PRIMARY_REGION].gcp
...
resource "google_storage_bucket" "usa" {
  ...
  location      = locals.region_mapping[var.CLOUD_SECONDARY_REGION].gcp
  storage_class = "STANDARD"
}
