...
O pipeline que orquestra este código no GitHub Actions está localizado em `../../.github/workflows/tofu-pipeline.yml`. O estado remoto (`tfstate`) é gerenciado pelo Terraform Cloud no modo CLI-driven. A autenticação na nuvem é feita via OIDC nativo do GitHub Actions, mantendo o princípio de Zero Trust.