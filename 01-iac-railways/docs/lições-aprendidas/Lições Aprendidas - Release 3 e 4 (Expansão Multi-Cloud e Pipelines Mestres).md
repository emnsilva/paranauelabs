A expansão da matriz para Azure, GCP e Oracle no GitHub revelou a necessidade de criar Pipelines Mestres e lidar com limitações das clouds.

## 1. O Pragmatismo do Pipeline Mestre (UX e DRY)
**O que aconteceu:** Eu tínha 8 arquivos .yml separados (tfc-aws, tofu-azure, etc), o que violava o princípio DRY e poluía a aba de Actions.<br>
...
**A solução:** Dividi em dois Pipelines Mestres (tfc-master-pipeline.yml e tofu-master-pipeline.yml). Usando os inputs (dropdowns) nativos do GitHub Actions e condicionais simples de Bash (if [ cloud == aws ]), montei o working-directory e o TF_WORKSPACE dinamicamente. Isso reduziu o código de CI/CD pela metade e melhorou a UX de quem opera a esteira.
...
**A solução:** Decisão arquitetual pragmática. Para a Oracle, mantive a autenticação via API Key (injetada via Variable Set do TFC como TF_VAR_private_key, etc). Isso manteve a esteira funcional e automatizada, reconhecendo que a OCI tem uma maturidade de integração diferente das outras clouds neste aspecto.
...
**A solução:** Adicionei a linha hostname = "app.terraform.io" no bloco cloud {} de todos os arquivos versions.tf do OpenTofu.
