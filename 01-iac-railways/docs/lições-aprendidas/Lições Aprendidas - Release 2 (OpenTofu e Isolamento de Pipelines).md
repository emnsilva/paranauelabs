A introdução do OpenTofu no GitHub Actions exigiu ajustes no arquivo de backend e na Trust Policy da AWS.
...
**A solução:** Adicionei a linha hostname = "app.terraform.io" dentro do bloco cloud {} no arquivo versions.tf do OpenTofu.
...
**A solução:** Atualizei a Trust Policy na AWS para aceitar o curinga workspace:dev_aws_*:run_phase:*. Isso permite que tanto o Terraform quanto o OpenTofu assumam a mesma Role, mantendo o isolamento dos states no TFC.

## 3. A diferença de UX: GitHub Actions não tem "Botão de Play" no meio do pipeline
**O que aconteceu:** No GitLab, eu usava when: manual para criar botões de Play (▶️) independentes para o Apply e o Destroy.
